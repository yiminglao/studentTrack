...
namespace ProjectGroup2
{
    public class clsSQLite
    {
		...
		public clsSQLite()
        {
            ...
            //sConnectionString = @"Data source=D:\feedme\FeedMe\StudentSyncV01.db; Version=3;";

            sConnectionString = @"Data source= " + Directory.GetCurrentDirectory() + "\\StudentSync.db; Version=3;";
        }
		...
		public DataSet ExecuteSQLStatement(string sSQL, ref int iRetVal)
        {
            try
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(sConnectionString))
                {
                    using (SQLiteDataAdapter adapter = new SQLiteDataAdapter())
                    {
                        ...
                        adapter.SelectCommand = new SQLiteCommand(sSQL, conn);
